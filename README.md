...
Production-ready Strapi 5.52.0 base image for HelmForge charts.

## Features

- **Strapi 5.52.0** - Packaged stable release
...
```bash
docker pull helmforge/strapi-base:5.52.0
...
```bash
docker run -e NODE_ENV=development helmforge/strapi-base:5.52.0
```

**Production** (optimized, schema fixed):

```bash
docker run -e NODE_ENV=production helmforge/strapi-base:5.52.0
```
